...
public interface MybatisStationArrivalDistanceRequestRepository {
    ...
    @Select("SELECT EXISTS(SELECT 1 FROM station_arrival_distance_data_request WHERE system_name = #{systemName} AND station_Name = #{stationName})")
    boolean exists(@Param("systemName") String systemName, @Param("stationName") String stationName);

    @Select("SELECT * FROM station_arrival_distance_data_request")
